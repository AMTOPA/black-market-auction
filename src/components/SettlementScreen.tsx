  ...
  return (
    <main className="screen grid">
      <section className="panel fade-in-up">
        <div className="panel-title">拍卖会 #{state.auctionNumber} · 场后结算</div>
      ...
      <section className="panel">
        <div className="panel-title">本场新闻与市场风声</div>
        {state.news.length > 0 ? (
          state.news.map((news) => (
      ...
      {specialBuyer ? (
        <section className="notice">
          <div className="bold gold">
            {specialBuyer.emoji} 特殊买家「{specialBuyer.name}」正在收货
          </div>
          <div>
            求购：{specialBuyer.wantCategory}
            {specialBuyer.wantTag ? ` · 偏爱「${specialBuyer.wantTag}」` : ""} · 报价倍率 {formatMult(specialBuyer.mult)}
          </div>
          <div className="small">{specialBuyer.blurb}</div>
        </section>
      ) : null}

      <section className="panel">
        <div className="panel-title">库存清单</div>
        {state.inventory.length > 0 ? (
          ...
          <div className="notice center">库存空空如也。保留现金，准备下一场机会。</div>
        )}
      </section>
      ...
      <section className="panel">
        <div className="btn-row">
          ...
          <button type="button" className="btn" onClick={handleHome}>
            回主页
          </button>
        </div>
      </section>
